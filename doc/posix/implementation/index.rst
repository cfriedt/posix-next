...
inside the notification function. ``SIGEV_THREAD_ID``, the extension Linux offers under
``_GNU_SOURCE``, is available there as well, with one difference: on Linux
``sigev_notify_thread_id`` carries a kernel thread ID (``gettid()``), whereas here it
carries a ``pthread_t`` value. A ``NULL`` ``evp`` behaves as POSIX specifies: ``SIGEV_SIGNAL`` with
