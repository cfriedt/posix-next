...
config POSIX_AEP_CHOICE_PSE53
	...
	help
	  ...
	  option groups POSIX_MULTI_PROCESS, POSIX_NETWORKING, POSIX_PIPE, and
	  POSIX_SIGNAL_JUMP (where the configured C library supports it; see
	  CONFIG_POSIX_SIGNAL_JUMP); and options _POSIX_CPUTIME,
	  _POSIX_PRIORITY_SCHEDULING, _POSIX_RAW_SOCKETS, _POSIX_SPAWN, and
...
config POSIX_AEP_REALTIME_DEDICATED
	...
	help
	  ...
	  POSIX_MULTI_PROCESS, POSIX_NETWORKING, POSIX_PIPE, POSIX_SIGNAL_JUMP
	  (where the configured C library supports it), _POSIX_CPUTIME,
	  _POSIX_PRIORITY_SCHEDULING, _POSIX_RAW_SOCKETS, _POSIX_SPAWN, and
	  _POSIX_SPORADIC_SERVER.
