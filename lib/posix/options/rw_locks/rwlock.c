			       ...
			       const struct timespec *abstime)
{
	...
	if (read_lock_acquire(rwl, sys_timepoint_timeout(timespec_abs_rt_to_timepoint(abstime))) !=
	    0U) {
		ret = ETIMEDOUT;
	}
			       ...
			       const struct timespec *abstime)
{
	...
	if (write_lock_acquire(rwl, sys_timepoint_timeout(timespec_abs_rt_to_timepoint(abstime))) !=
	    0U) {
		ret = ETIMEDOUT;
	}
