    ...
    def _build_group_state(self) -> None:
        for gid, info in self.option_group_symbols.items():
            ...
            if total == 0:
                ...
                pct = sum(line_pcts) / float(total)
                branch_pct = sum(branch_pcts) / float(total)
