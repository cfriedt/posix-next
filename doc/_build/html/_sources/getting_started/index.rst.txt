...
``posix-next`` maintains a series of patches on top of the upstream Zephyr tree.
They are registered in ``modules/lib/posix/zephyr/patches.yml`` and must be
applied before building anything:
...
.. note::
   ...
   The patches must be re-applied whenever the Zephyr tree is updated or
   otherwise reset. To do so cleanly:
