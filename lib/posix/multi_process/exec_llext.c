...
/* a slot claimed ahead of the load, so a full table fails before any heap is spent */
#define EXEC_IMAGE_PENDING ((struct llext *)-1)
...
static struct llext *exec_image_detach(k_pid_t pid, struct llext *skip)
{
	...
	K_SPINLOCK(&exec_images_lock) {
		for (size_t i = 0; i < ARRAY_SIZE(exec_images); i++) {
			...
			if (exec_images[i].ext == EXEC_IMAGE_PENDING) {
				/* died mid-exec: nothing loaded yet */
...
static int exec_image_claim(k_pid_t self)
{
	...
	for (int pass = 0; pass < 2; pass++) {
		...
		K_SPINLOCK(&exec_images_lock) {
			for (size_t i = 0; i < ARRAY_SIZE(exec_images); i++) {
				if (exec_images[i].ext == NULL) {
					...
					exec_images[i].ext = EXEC_IMAGE_PENDING;
					claimed = (int)i;
					break;
				}
			}
		...
		for (size_t i = 0; (pass == 0) && (i < ARRAY_SIZE(exec_images)); i++) {
			...
			if ((stale != NULL) && (stale != EXEC_IMAGE_PENDING)) {
...
static void exec_image_commit(int slot, struct llext *ext)
{
	K_SPINLOCK(&exec_images_lock) {
		exec_images[slot].ext = ext;
	}
}
