...
``sys_clone()`` is the creation primitive; :c:func:`fork`, ``vfork``, and
``posix_spawn`` are library constructs over it, mirroring Linux. With no flags
it produces a fresh process from a pooled record and a pool-drawn leader
thread. ``SYS_CLONE_PAUSED`` leaves the leader stopped so a caller can apply
attributes (process group, signal mask, scheduling) before starting it.
``SYS_CLONE_VM_COPY`` selects the fork model.
...
.. graphviz::
   ...
   digraph clone {
       ...
       fork [label="process_clone_vm_copy\n(duplicate address space,\nresume via longjmp)"];
...
The substrate is experimental (:kconfig:option:`CONFIG_PROCESS_VM`, x86 only).
...
.. note::
   **Confirmed working (x86_64, ia32, and arm64):** fork() returns twice through
   the kernel resume, the child diverges on its own stack and data copies,
   and the parent - whose view is asserted unchanged - reaps it. On arm64
   no dedicated resume path was needed at all: every arm64 thread is born
