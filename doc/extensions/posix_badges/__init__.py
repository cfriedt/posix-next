...
def on_env_get_outdated(app: Sphinx, env, added, changed, removed):
    ...
    return [
        ...
        if docname.startswith(OPTION_GROUPS_PREFIX) and docname not in skip
    ]
...
def on_doctree_resolved(app: Sphinx, doctree, docname):
    ...
    if _DB is None or not docname.startswith(OPTION_GROUPS_PREFIX):
        return
    stem = docname[len(OPTION_GROUPS_PREFIX):]
    if stem == "index":
        return
    group = _DB["groups"].get(stem)
    if not group:
        return
    strip = render.badge_strip_html(stem, group, _THRESHOLDS)
    if not strip:
        return
...
def on_build_finished(app: Sphinx, exception):
    ...
    group_pages = {
        doxy_html / f"group__{f'posix_option_group_{stem}'.replace('_', '__')}.html": stem
    ...
    for page in sorted(doxy_html.glob("group__*.html")):
        ...
        stem = group_pages.get(page)
        scenarios = None
        if stem:
            scenarios = _DB["groups"][stem].get("scenarios")
            strip = render.badge_strip_svg(stem, _DB["groups"][stem], _THRESHOLDS)
            if strip:
