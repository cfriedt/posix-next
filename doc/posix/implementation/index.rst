...
tickless wakeups are counted correctly). ``SIGEV_THREAD`` is implemented on top of
thread-directed delivery, as glibc does: a dedicated helper thread is created (detached) at
:c:func:`timer_create` time and woken by an internal signal number just past ``SIGRTMAX``,
which applications can neither send, mask, nor wait on; :c:func:`timer_getoverrun` is valid
...
**Reduced mode.** When the kernel signal subsystem (:kconfig:option:`CONFIG_SIGNAL`) is
unavailable, ``SIGEV_NONE`` timers remain fully functional as time sources, while
``SIGEV_SIGNAL``, ``SIGEV_THREAD``, and ``SIGEV_THREAD_ID`` report ``ENOTSUP`` from
:c:func:`timer_create`. All AEP profiles select the signal subsystem.

Migration from earlier releases: ``CONFIG_POSIX_TIMER_MAX`` was replaced by
:kconfig:option:`CONFIG_SYS_TIMER_MAX` (which ``TIMER_MAX`` and ``sysconf(_SC_TIMER_MAX)``
now report), guaranteed capacity moved to the distributed ``CONFIG_SYS_TIMER_MIN_ADD_<NAME>``
minimum, and ``CONFIG_TIMER_CREATE_WAIT`` was removed because :c:func:`timer_create` no
longer blocks.
