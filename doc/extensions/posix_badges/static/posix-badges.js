...
(function () {
  ...
  function onOptionGroupsIndex() {
    return /\/option_groups\/(index\.html)?$/.test(window.location.pathname);
  }
  ...
  function decorateFunctions(db) {
    ...
    var scenarios = ((db.groups || {})[stem] || {}).scenarios || {};
  ...
  /* Replace the index toctree bullet list with a table like the detail
     pages: an Option Group link column plus an aligned Status column. */
  function decorateIndex(db) {
    if (!onOptionGroupsIndex()) {
      return;
    }
    var wrapper = document.querySelector('[role="main"] .toctree-wrapper');
    if (!wrapper) {
      return;
    }
    ...
    items.forEach(function (item) {
      ...
      SCENARIO_BADGES.forEach(function (entry) {
        var s = scen[entry[0]];
        if ((s === "passed" || s === "failed") && !scenarioSeen[entry[0]]) {
    ...
    ["Option Group", "Status"].forEach(function (text) {
      var th = document.createElement("th");
      th.className = "head";
      th.textContent = text;
      headRow.appendChild(th);
  ...
  function run() {
    ...
    if (!needsDetail && !onOptionGroupsIndex()) {
      return;
    }
    fetchDb()
      .then(function (db) {
