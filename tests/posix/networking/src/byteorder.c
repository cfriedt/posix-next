...
ZTEST_USER(posix_networking, test_htonl)
{
...
ZTEST_USER(posix_networking, test_htons)
{
...
ZTEST_USER(posix_networking, test_ntohl)
{
	zassert_equal(ntohl(0x04030201U), 0x01020304U);
	zassert_equal(ntohl(0U), 0U);
}

ZTEST_USER(posix_networking, test_ntohs)
{
