...
static void semaphore_test(sem_t *sem)
{
	...
	/* TESTPOINT: Initialize sema with 0 and block a waiter for EBUSY */
	zassert_equal(sem_init(sem, 0, 0), 0, "sem_init failed");
	...
	/* TESTPOINT: Wait and acquire semaphore till thread3 gives */
	zassert_equal(sem_wait(sem), 0, "sem_wait failed");
...
{
	/* checks below rely on Zephyr-internal named semaphore accounting */
