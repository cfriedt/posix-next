...
(function () {
  ...
  function setDiagramLinkHref(el, tabId) {
    const hash = "#" + normalizeId(tabId);
    el.setAttributeNS("http://www.w3.org/1999/xlink", "href", hash);
    el.setAttribute("href", hash);
  }

  function wireAepDiagramLink(el) {
    const tabId = el.getAttribute("data-aep-tab");
    if (!tabId) {
      return;
    }

    setDiagramLinkHref(el, tabId);
  ...
  async function inlineAepDiagram(container) {
    ...
    holder.querySelectorAll("[data-aep-tab]").forEach(wireAepDiagramLink);
