...
def _iter_functions(text: str):
    """Yield (name, body) for each top-level function definition in C text.
    ...
    by requiring ')' immediately before '{' and a parseable name.
    """
    depth = 0
    body_start = None
    sig_end = 0
...
def function_coverage(coverage_json: dict) -> dict:
    """Function -> (line_hits, line_total, repo-relative file, first line)."""
    """Function name -> (line_hits, line_total) across lib/posix sources.

    gcovr snapshots sometimes lose function names (recorded as
    ``<unknown function N>``), typically in small single-function files.
    When a file is named ``<function>.c`` and carries no *named* attribution
    at all, its whole-file line stats are credited to that function; files
    with any real named attribution are never fallback-attributed.
    """
    ...
    for entry in coverage_json.get("files", []):
        ...
        if file_lines and all(_UNKNOWN_FN_RE.match(n) for n in names):
...
def load_twister_summary(path: Path, aliases: dict):
    """Return (provenance | None, {canonical key: {variant: ScenarioSummary}})."""
...
def load_metrics(repo_root: Path | None = None) -> PosixMetrics:
    ...
    twister_provenance, scenarios = load_twister_summary(
    ...
    for stem, roster in rosters.items():
        ...
        groups[key] = GroupMetrics(
            ...
            scenarios=scenarios.get(key) if twister_provenance else None,
        )
    ...
    fn_cov = function_coverage(coverage_json) if coverage_json else {}
