...
static void test_pthread_condattr(void)
{
	...
	/* degenerate cases */
	IF_NOT_NATIVE_LIBC({
		zassert_equal(pthread_condattr_init(NULL), EINVAL);
		zassert_equal(pthread_condattr_destroy(NULL), EINVAL);
	})
...
static void test_cond_init_existing_initialized_condattr(void)
{
	...
	IF_NOT_NATIVE_LIBC({
		/* an uninitialized condattr is rejected */
...
static void test_pthread_cond_timedwait(void)
{
	...
	/* the first wait on a statically-initialized cond allocates it */
	{
		int rc;
		...
		zassert_ok(pthread_mutex_lock(&cond_wait_mtx));
		rc = pthread_cond_timedwait(&static_cv, &cond_wait_mtx, &past);
		zassert_equal(rc, ETIMEDOUT, "expected ETIMEDOUT, got %d", rc);
		zassert_ok(pthread_mutex_unlock(&cond_wait_mtx));
