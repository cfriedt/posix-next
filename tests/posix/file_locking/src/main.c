...
static uint32_t now_ms(void)
{
	...
	return ts.tv_sec * USEC_PER_SEC + ts.tv_nsec / NSEC_PER_MSEC;
}
...
static void *flockfile_thread_entry(void *arg)
{
	...
	if (ctx->delay_ms > 0) {
		zassert_ok(msleep(ctx->delay_ms));
	}
...
static int flockfile_retry(FILE *fp, bool retry, int delay_ms)
{
	while (true) {
...
static void *ftrylockfile_thread_entry(void *arg)
{
	...
	for (int i = 0; i < ctx->lock_count; ++i) {
		int ret = flockfile_retry(ctx->fp, ctx->retry, ctx->delay_ms);
...
static void flockfile_common(bool try)
{
	...
	if (false) {
		/* degenerate cases */
		flockfile_retry(NULL, try, 0); /* this will throw an assertion if enabled */
	}
	...
	zexpect_true(now - then <= CONFIG_TEST_NEGLIGIBLE_DELAY_MS, "delay of %d ms exceeds %d ms",
		     (int)(now - then), CONFIG_TEST_NEGLIGIBLE_DELAY_MS);
	...
	now = now_ms();
	zexpect_true((now - then) >= 2 * CONFIG_TEST_LOCK_PERIOD_MS,
		     "delay of %u ms less than %u ms", (int)(now - then),
		     2 * CONFIG_TEST_LOCK_PERIOD_MS);
	...
	now = now_ms();
	zexpect_true((now - then) >= 4 * CONFIG_TEST_LOCK_PERIOD_MS,
		     "delay of %u ms less than %u ms", (now - then),
...
ZTEST(posix_file_locking, test_ftrylockfile)
{
	...
	now = now_ms();
	zexpect_true((now - then) >= CONFIG_TEST_LOCK_PERIOD_MS, "delay of %u ms less than %u ms",
...
ZTEST(posix_file_locking, test_getchar_unlocked)
{
	...
	/* Host libc blocks on stdin when it is a TTY; twister does not redirect it. */
	zassert_not_null(freopen("/dev/null", "r", stdin));
#endif
	flockfile(stdin);
