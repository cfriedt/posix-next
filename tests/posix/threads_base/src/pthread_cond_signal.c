...
/* whose turn it is: set by the thread handing over, consumed by exactly one waiter */
static ZTEST_BMEM int bounce_turn;
...
static void *thread_top_exec(void *p1)
{
	...
	for (i = 0; i < BOUNCES; i++) {
		...
		if (i == 0) {
			...
			/*
			 * Wait on a predicate: a signal only wakes threads blocked on the
			 * condition variable, and a peer may still be blocked on the mutex.
			 */
			while (!bounce_turn) {
				zassert_equal(0, pthread_cond_wait(&cvar0, &lock), "");
			}
