...
Mandatory POSIX :ref:`System Interfaces<posix_system_interfaces_required>` can also be enabled
on their own via :kconfig:option:`CONFIG_POSIX_AEP_CHOICE_BASE`.

.. note::
   In the *Option Groups* tables below, a blank Support cell with a Kconfig remark means the
...
:ref:`System Interfaces<posix_system_interfaces_required>`. POSIX tracing
appeared in IEEE 1003.13 but was removed from the specification in Issue 8 and is not listed here
as a PSE52 option requirement :ref:`†<posix_undefined_behaviour>`.
