...
#define DYNAMIC_STACK_SIZE CONFIG_SYS_THREAD_STACK_SIZE
		   ...
		   void *(*start_routine)(void *), void *ZRESTRICT arg)
{
	...
	if (!__attr_is_initialized(attrp)) {
		return EINVAL;
	}
	...
	int ret =
		-sys_thread_create(&k_thread, attrp->stack, attrp->stacksize, zephyr_thread_wrapper,
				   arg, start_routine, NULL, prio, options);
